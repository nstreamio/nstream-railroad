...
import { Lazy, Arrays, Equals, Values, Timing, Easing } from "@swim/runtime";

import { Format } from "@swim/runtime";

import { DateTime } from "@swim/runtime";

import { Trait, Model, ModelRef, TraitSet, TraitRef, SelectableTrait } from "@swim/toolkit";

import { Property, Affinity, Provider, Service, Animator } from "@swim/runtime";

import { ViewRef, ViewSet, View, ViewportProvider, ViewportService, ThemeService, PositionGesture } from "@swim/toolkit";

import { HtmlView } from "@swim/toolkit";

import { Look, Feel, ThemeAnimator, Theme, MoodVector, Mood, MoodMatrix } from "@swim/toolkit";

import { TraitViewRef, TraitViewControllerSet, Controller, TraitControllerSet, ControllerRef, ControllerSet, HistoryProvider, HistoryService } from "@swim/toolkit";

import { TextCellController, IconCellController, RowController, TextCellTrait, IconCellTrait, TableController, TableTrait, RowView, IconCellView, TextCellView, TableView, TableLayout, ColLayout, LeafView, DisclosureCellView, ColView, HeaderView } from "@swim/toolkit";

import { TypesetView, CanvasView, VectorIcon, HtmlIconView, Icon, FilledIcon, EnclosedIcon, PolygonIcon, CircleIcon } from "@swim/toolkit";

import { DialController, GaugeController, GaugeTrait } from "@swim/toolkit";

import { Angle, Length, R2Path } from "@swim/runtime";

import { SliceController, PieController, PieTrait } from "@swim/toolkit";

import { BubblePlotController, LinePlotController, AreaPlotController, ChartController, BubblePlotTrait, LinePlotTrait, AreaPlotTrait, ChartTrait } from "@swim/toolkit";

import { InputTokenView } from "@swim/toolkit";

import { PopoverView, DrawerView, ScrimView, DrawerButton } from "@swim/toolkit";

import { Expansion, LinearGradient, ColorStop, Color } from "@swim/toolkit";

import { IconButton, ButtonMembrane } from "@swim/toolkit";

import { ConstraintProperty, ConstraintGroup } from "@swim/runtime";

import { ValueDownlinkFastener, MapDownlinkFastener, EventDownlinkFastener, ListDownlinkFastener } from "@swim/runtime";

import { Uri, UriPath, UriFragment, UriUser, UriAuthority, UriScheme } from "@swim/runtime";

import { GeoJsonPoint, GeoJsonMultiPoint, GeoPath, GeoJsonLineString, GeoJsonMultiLineString, GeoJsonPolygon, GeoJsonMultiPolygon, GeoPoint, GeoGroup, GeoJson } from "@swim/runtime";

import { GeoView, WorldMapView, GeoIconView, GeoLineView, GeoAreaView, GeoCircleView, GeoRasterView } from "@swim/toolkit";

import { MapboxView } from "@swim/toolkit";

import { DeckView, TitleDeckView, TitleDeckBar, DeckCard } from "@swim/toolkit";

import { Attr, Record, Form } from "@swim/runtime";

import { Recon } from "@swim/runtime";
...
__decorate([ Lazy ], StatusVector, "empty", null);
...
class StatusTrait extends Trait {
  constructor() {
    super();
...
StatusTrait.MountFlags = Trait.MountFlags | Model.NeedsAggregate;
...
StatusGroup.MountFlags = Model.MountFlags | Model.NeedsAggregate;

StatusGroup.InsertChildFlags = Model.InsertChildFlags | Model.NeedsAggregate;

StatusGroup.RemoveChildFlags = Model.RemoveChildFlags | Model.NeedsAggregate;
...
__decorate([ Lazy ], IndicatorMap, "empty", null);

class IndicatorTrait extends Trait {
...
IndicatorGroup.MountFlags = StatusGroup.MountFlags | Model.NeedsAggregate | Model.NeedsReconcile;

IndicatorGroup.InsertChildFlags = StatusGroup.InsertChildFlags | Model.NeedsAggregate | Model.NeedsReconcile;

IndicatorGroup.RemoveChildFlags = StatusGroup.RemoveChildFlags | Model.NeedsAggregate | Model.NeedsReconcile;

class IndicatedTrait extends Trait {
  get indicatorMap() {
...
IndicatedTrait.MountFlags = Trait.MountFlags | Model.NeedsAggregate | Model.NeedsReconcile;
...
IndicatedGroup.MountFlags = StatusGroup.MountFlags | Model.NeedsAggregate | Model.NeedsReconcile;

IndicatedGroup.InsertChildFlags = StatusGroup.InsertChildFlags | Model.NeedsAggregate | Model.NeedsReconcile;

IndicatedGroup.RemoveChildFlags = StatusGroup.RemoveChildFlags | Model.NeedsAggregate | Model.NeedsReconcile;
...
class DomainTrait extends Trait {
...
class WidgetView extends HtmlView {
...
class WidgetTrait extends Trait {
...
class WidgetGroup extends Trait {}
...
class TableGadgetTextCellController extends TextCellController {
...
__decorate([ TraitViewRef({
  ...
  didAttachTrait(cellTrait, targetTrait) {
    ...
    TextCellController.cell.prototype.didAttachTrait.call(this, cellTrait, targetTrait);
  },
  willDetachTrait(cellTrait) {
    TextCellController.cell.prototype.willDetachTrait.call(this, cellTrait);
    this.owner.status.setTrait(null);
  },
  initView(cellView) {
    TextCellController.cell.prototype.initView.call(this, cellView);
...
class TableGadgetIconCellController extends IconCellController {
  applyStatusVector(statusVector, cellView) {
    const alert = statusVector.get(Status.alert) || 0;
    const warning = statusVector.get(Status.warning) || 0;
    const inactive = statusVector.get(Status.inactive) || 0;
...
class TableGadgetRowController extends RowController {
...
class TableGadgetController extends TableController {
  constructor() {
    super();
...
class GaugeGadgetDialController extends DialController {
...
class GaugeGadgetController extends GaugeController {
...
class PieGadgetSliceController extends SliceController {
...
class PieGadgetController extends PieController {
  layoutPie(hasLegend) {
...
class ChartGadgetBubblePlotController extends BubblePlotController {
...
__decorate([ TraitViewRef({
  ...
  didAttachTrait(plotTrait, targetTrait) {
    ...
    BubblePlotController.plot.prototype.didAttachTrait.call(this, plotTrait, targetTrait);
  },
  willDetachTrait(plotTrait) {
    BubblePlotController.plot.prototype.willDetachTrait.call(this, plotTrait);
    this.owner.status.setTrait(null);
  },
  initView(plotView) {
    BubblePlotController.plot.prototype.initView.call(this, plotView);
    const statusTrait = this.owner.status.trait;
    if (statusTrait !== null) {
      plotView.modifyMood(Feel.default, [ [ Feel.primary, 1 ] ]);
      this.owner.applyStatusVector(statusTrait.statusVector, plotView);
    }
    plotView.fill.setLook(Look.accentColor, Affinity.Intrinsic);
  }
...
class ChartGadgetLinePlotController extends LinePlotController {
  applyStatusVector(statusVector, plotView) {
    const alert = statusVector.get(Status.alert) || 0;
    const warning = statusVector.get(Status.warning) || 0;
    const inactive = statusVector.get(Status.inactive) || 0;
...
__decorate([ TraitViewRef({
  ...
  didAttachTrait(plotTrait, targetTrait) {
    this.owner.status.setTrait(plotTrait.getTrait(StatusTrait));
    LinePlotController.plot.prototype.didAttachTrait.call(this, plotTrait, targetTrait);
  },
  willDetachTrait(plotTrait) {
    LinePlotController.plot.prototype.willDetachTrait.call(this, plotTrait);
    this.owner.status.setTrait(null);
  },
  initView(plotView) {
    LinePlotController.plot.prototype.initView.call(this, plotView);
    const statusTrait = this.owner.status.trait;
    if (statusTrait !== null) {
      plotView.modifyMood(Feel.default, [ [ Feel.primary, 1 ] ]);
      this.owner.applyStatusVector(statusTrait.statusVector, plotView);
...
class ChartGadgetAreaPlotController extends AreaPlotController {
  applyStatusVector(statusVector, plotView) {
    const alert = statusVector.get(Status.alert) || 0;
    const warning = statusVector.get(Status.warning) || 0;
    const inactive = statusVector.get(Status.inactive) || 0;
...
__decorate([ TraitViewRef({
  ...
  didAttachTrait(plotTrait, targetTrait) {
    this.owner.status.setTrait(plotTrait.getTrait(StatusTrait));
    AreaPlotController.plot.prototype.didAttachTrait.call(this, plotTrait, targetTrait);
  },
  willDetachTrait(plotTrait) {
    AreaPlotController.plot.prototype.willDetachTrait.call(this, plotTrait);
    this.owner.status.setTrait(null);
  },
  initView(plotView) {
    AreaPlotController.plot.prototype.initView.call(this, plotView);
    const statusTrait = this.owner.status.trait;
    if (statusTrait !== null) {
      plotView.modifyMood(Feel.default, [ [ Feel.primary, 1 ] ]);
      this.owner.applyStatusVector(statusTrait.statusVector, plotView);
...
class ChartGadgetController extends ChartController {}

__decorate([ TraitViewRef({
  extends: true,
  initView(chartView) {
    ChartController.chart.prototype.initView.call(this, chartView);
...
__decorate([ TraitViewControllerSet({
  ...
  createController(plotTrait) {
    ...
    if (plotTrait instanceof BubblePlotTrait) {
      ...
      plotController = ChartController.plots.prototype.createController.call(this, plotTrait);
    }
...
class PrismService extends Service {
  constructor() {
    super();
    this.plugins = [];
  }
...
__decorate([ Lazy ], PrismService, "global", null);
...
class SurfaceView extends HtmlView {}
...
__decorate([ Lazy ], CollectionController, "domainIcon", null);
...
__decorate([ Lazy ], CollectorController, "searchIcon", null);

__decorate([ Lazy ], CollectorController, "plusIcon", null);
...
__decorate([ TraitViewRef({
  ...
  initView(widgetView) {
    ...
    widgetView.marginTop.setState(4, Affinity.Intrinsic);
...
class MagnifierHandle extends ButtonMembrane {
  constructor(node) {
    super(node);
  ...
  initHandle() {
    ...
    this.marginBottom.setState(4, Affinity.Intrinsic);
...
__decorate([ ViewRef({
  ...
  fromAny(value) {
    if (value instanceof HtmlView) {
      ...
      iconView.graphics.setState(value, Affinity.Intrinsic);
      return iconView;
    }
  }
...
__decorate([ ViewRef({
  ...
  fromAny(value) {
    if (value instanceof HtmlView) {
      ...
      accessoryView.graphics.setState(value, Affinity.Intrinsic);
      return accessoryView;
    }
  }
}) ], MagnifierHandle.prototype, "accessory", void 0);

class MagnifierController extends Controller {
  constructor() {
    super();
...
__decorate([ ViewRef({
  ...
  createView() {
    ...
    fixturesView.flexGrow.setState(1, Affinity.Intrinsic);
...
__decorate([ Lazy ], MagnifierController, "collapseIcon", null);

class ActivityWindow extends PopoverView {
  constructor(node) {
    super(node);
    this.initWindow();
    this.initTheme();
...
class BeamView extends HtmlView {
  constructor(node) {
    super(node);
    this.initBeam();
  }
...
class ShellView extends HtmlView {
  constructor(node) {
    super(node);
    ...
    if (surfaceView !== null) {
      ...
      if (backgroundColor !== null) {
        ...
        if (surfaceView.fullBleed.value) {
          gradient = LinearGradient.create("bottom", ColorStop.create(backgroundColor.alpha(0), 0), ColorStop.create(backgroundColor.alpha(0), 100, 75));
        } else {
          gradient = LinearGradient.create("bottom", ColorStop.create(backgroundColor, 0), ColorStop.create(backgroundColor.alpha(0), 100, 75));
        }
    ...
    if (surfaceView !== null) {
      ...
      if (backgroundColor !== null) {
        ...
        if (surfaceView.fullBleed || surfaceView.viewportIdiom !== "mobile") {
          gradient = LinearGradient.create("top", ColorStop.create(backgroundColor.alpha(0), 0), ColorStop.create(backgroundColor.alpha(0), 100, 75));
        } else {
          gradient = LinearGradient.create("top", ColorStop.create(backgroundColor, 0), ColorStop.create(backgroundColor.alpha(0), 100, 75));
        }
...
class ShellController extends Controller {
  ...
  applyStatus(statusVector) {
    ...
    if (beamView !== null) {
      ...
      const primaryMood = MoodVector.of([ Feel.primary, 1 ], [ Feel.darker, .5 ]);
      const warningMood = MoodVector.of([ Feel.warning, 1 ], [ Feel.darker, .5 ]);
      const alertMood = MoodVector.of([ Feel.alert, 1 ], [ Feel.darker, .5 ]);
      const inactiveMood = MoodVector.of([ Feel.inactive, 1 ], [ Feel.darker, .5 ]);
      let leftStop;
      let inactiveStop;
      let normalStop;
      let warningStop;
  ...
  static get leftListLayout() {
    return TableLayout.create([ ColLayout.create("icon", 0, 0, 70, false, true), ColLayout.create("title", 1, 0, 0, false, false, Look.color) ]);
  }
...
ShellController.MountFlags = Controller.MountFlags | Controller.NeedsRevise;
...
__decorate([ ViewRef({
  ...
  initView(rowView) {
    ...
    rowView.getOrCreateCell("icon", IconCellView).iconWidth(70, Affinity.Intrinsic).iconHeight(44, Affinity.Intrinsic).graphics(this.owner.brandIcon, Affinity.Intrinsic);
...
__decorate([ ViewRef({
  ...
  initView(rowView) {
    rowView.modifyMood(Feel.default, [ [ Feel.unselected, 1 ] ], false);
    rowView.userSelect("none").hovers(true).rowHeight(48).layout(ShellController.leftListLayout);
    rowView.getOrCreateCell("icon", IconCellView).iconWidth(20, Affinity.Intrinsic).iconHeight(20, Affinity.Intrinsic).graphics(this.owner.settingsIcon, Affinity.Intrinsic);
    rowView.getOrCreateCell("title", TextCellView).content("Settings");
  }
...
__decorate([ Lazy ], ShellController, "leftListLayout", null);

__decorate([ Lazy ], ShellController, "brandIcon", null);

__decorate([ Lazy ], ShellController, "settingsIcon", null);

__decorate([ Lazy ], ShellController, "inventoryIcon", null);

class PulseTrait extends Trait {
...
__decorate([ ValueDownlinkFastener({
  nodeUri() {
...
__decorate([ MapDownlinkFastener({
  consumed: true,
  nodeUri() {
    return this.owner.metaHostUri;
  },
  laneUri() {
    return Uri.create(void 0, void 0, UriPath.segment("nodes"), void 0, UriFragment.create(this.owner.nodePath.toString()));
  },
  didUpdate(key, value) {
...
class PlaneEntity extends EntityTrait {
  ...
  onAttachModel(model) {
    super.onAttachModel(model);
    if (model.warpRef.hasAffinity(Affinity.Intrinsic)) {
      ...
      model.warpRef.setValue(warpRef, Affinity.Intrinsic);
    }
    this.subentities.insertModel();
  }
}
...
__decorate([ Lazy ], NodeGroup, "metaHostUri", null);
...
class FabricPlugin extends DomainPlugin {
  ...
  createDomain(hostUri) {
    ...
    entityTrait.title.setValue(hostUri.authority.withUser(UriUser.undefined()).toString());
...
__decorate([ Lazy ], PortalPlugin, "icon", null);

class CatalogIndicatorCell extends Controller {
...
class CatalogEntityLimb extends Controller {
  ...
  onInsertIndicator(childIndicator, targetIndicator) {
    ...
    let controllerRef = this.getFastener(key, ControllerRef);
  ...
  onRemoveIndicator(childIndicator) {
    const key = childIndicator.indicatorType.key;
    const controllerRef = this.getFastener(key, ControllerRef);
    if (controllerRef !== null) {
...
CatalogEntityLimb.IndicatorFastener = ControllerRef.define("IndicatorFastener", {
...
__decorate([ ViewRef({
  ...
  createView() {
    const icon = EnclosedIcon.embossed(PolygonIcon.create(6), null).withInnerScale(Math.sqrt(2) / 2);
    const iconCell = IconCellView.create().iconWidth(40).iconHeight(40).graphics(icon);
    return iconCell;
  }
...
class CatalogEntityTree extends Controller {
  ...
  onInsertEntity(childEntity, targetEntity) {
    ...
    let controllerRef = this.getFastener(id, ControllerRef);
    if (controllerRef === null) {
  ...
  onRemoveEntity(childEntity) {
    const id = childEntity.uri.toString();
    const controllerRef = this.getFastener(id, ControllerRef);
    if (controllerRef !== null) {
      controllerRef.deleteController();
  ...
  lookupBranch(treeView, key) {
    ...
    while (lo <= hi && !(childNodes[lo].view instanceof RowView)) {
      lo += 1;
    }
    while (hi >= lo && !(childNodes[hi].view instanceof RowView)) {
      hi -= 1;
    }
    while (lo <= hi) {
      const mid = lo + hi >>> 1;
      const order = Values.compare(key, childNodes[mid].view.key);
      if (order > 0) {
        lo = mid + 1;
      } else if (order < 0) {
        hi = mid - 1;
...
CatalogEntityTree.BranchRef = ControllerRef.define("BranchRef", {
...
__decorate([ Lazy ], CatalogPlugin, "icon", null);
...
__decorate([ Lazy ], DiagramPlugin, "icon", null);
...
class GeographicPoint extends Geographic {
  ...
  equals(that) {
    if (this === that) {
      ...
      return this.geometry.equals(that.geometry) && Equals(this.width, that.width) && Equals(this.height, that.height) && Equals(this.graphics, that.graphics) && Equals(this.fill, that.fill);
    }
    return false;
  }
  static fromInit(init) {
...
class GeographicLine extends Geographic {
  ...
  equals(that) {
    if (this === that) {
      ...
      return this.geometry.equals(that.geometry) && Equals(this.stroke, that.stroke) && Equals(this.strokeWidth, that.strokeWidth);
    }
    return false;
  }
  static fromInit(init) {
...
class GeographicArea extends Geographic {
  ...
  equals(that) {
    if (this === that) {
      ...
      return this.geometry.equals(that.geometry) && Equals(this.fill, that.fill) && Equals(this.stroke, that.stroke) && Equals(this.strokeWidth, that.strokeWidth);
    }
    return false;
  }
  static fromInit(init) {
...
const GeographicView = function() {
  ...
  GeographicView.is = function(object) {
    if (typeof object === "object" && object !== null) {
      const view = object;
      return view instanceof GeographicPoint || view instanceof GeographicLine || view instanceof GeographicArea || view instanceof GeographicGroup || view instanceof GeoView && "highlight" in view && "unhighlight" in view && "setState" in view;
    }
    return false;
  };
...
class GeographicPointView extends GeoIconView {
  get icon() {
...
class GeographicLineView extends GeoLineView {
  highlight(timing) {
    if (!this.highlighted.value) {
      if (timing === void 0 || timing === true) {
        timing = this.getLookOr(Look.timing, false);
  ...
  onHighlight(timing) {
    ...
    if (accentColor !== void 0 && this.stroke.hasAffinity(Affinity.Intrinsic)) {
  ...
  onApplyTheme(theme, mood, timing) {
    ...
    if (accentColor !== null && this.stroke.hasAffinity(Affinity.Intrinsic)) {
...
class GeographicAreaView extends GeoAreaView {
  highlight(timing) {
    if (!this.highlighted.value) {
      if (timing === void 0 || timing === true) {
        timing = this.getLookOr(Look.timing, false);
...
class GeographicGroupView extends GeoView {
  highlight(timing) {
    if (!this.highlighted.value) {
      if (timing === void 0 || timing === true) {
        timing = this.getLookOr(Look.timing, false);
...
class LocationTrait extends Trait {
  constructor() {
    super();
...
class DistrictTrait extends Trait {
  constructor() {
    super();
    this.minZoom = -Infinity;
    this.maxZoom = Infinity;
...
class AtlasEntityLocation extends Controller {
    ...
    if (geographicView !== null) {
      ...
      if (alert !== 0) {
        ...
        if (!oldStatusVector.isEmpty()) {
          this.ripple(geographicView.getLook(Look.accentColor), 2, 5e3);
        }
      } else if (warning !== 0) {
  ...
  ripple(color, width, timing) {
    ...
    if (geographicView !== null && geographicView.mounted) {
      if (geographicView instanceof GeoCircleView || geographicView instanceof GeoIconView || geographicView instanceof GeoAreaView) {
  ...
  onSetGeographic(geographic) {
    if (geographic !== null) {
      ...
      if (geographicView === null) {
        ...
        geographicView.setState(geographic, Easing.linear.withDuration(5e3));
      }
      const layerView = this.layer.view;
      if (layerView !== null) {
...
class AtlasEntityDistrict extends Controller {
...
class AtlasMap extends Controller {}

class AtlasWorldMap extends AtlasMap {
  createWorld() {
    const worldView = new GeoRasterView;
    const FillAnimator = ThemeAnimator.define("fill", {
      type: Color
    });
    const fillAnimator = FillAnimator.create(worldView);
    fillAnimator.setLook(Look.subduedColor, Affinity.Intrinsic);
    worldView.setFastener("fill", fillAnimator);
    function addWorldGeometry(shapes, mapView) {
      for (let i = 0; i < shapes.length; i += 1) {
        const shape = shapes[i];
        if (shape instanceof GeoGroup) {
          const layerView = mapView.appendChild(GeoView);
          addWorldGeometry(shape.shapes, layerView);
        } else if (shape instanceof GeoPath) {
          const shapeView = mapView.appendChild(GeoAreaView);
          shapeView.geoPath.setState(shape);
          shapeView.fill.setInherits(true);
        }
      }
    }
    const worldGeometry = GeoJson.toShape({
...
class AtlasMapbox extends AtlasMap {
  ...
  layoutSurface(surfaceView, viewContext) {
    ...
    if (containerView !== null) {
      ...
      const controlContainer = HtmlView.fromNode(containerView.node.querySelector(".mapboxgl-control-container"));
      const topLeftControls = HtmlView.fromNode(controlContainer.node.querySelector(".mapboxgl-ctrl-top-left"));
      topLeftControls.top(surfaceView.paddingTop.state);
      topLeftControls.left(left);
      const topRightControls = HtmlView.fromNode(controlContainer.node.querySelector(".mapboxgl-ctrl-top-right"));
      topRightControls.top(surfaceView.paddingTop.state);
      topRightControls.right(right);
      const bottomLeftControls = HtmlView.fromNode(controlContainer.node.querySelector(".mapboxgl-ctrl-bottom-left"));
      bottomLeftControls.bottom(surfaceView.paddingBottom.state);
      bottomLeftControls.left(left);
      const bottomRightControls = HtmlView.fromNode(controlContainer.node.querySelector(".mapboxgl-ctrl-bottom-right"));
...
class AtlasLayers extends Controller {
  get layersIcon() {
  ...
  static get layersListLayout() {
    return TableLayout.create([ ColLayout.create("icon", 0, 0, 60, false, true), ColLayout.create("title", 1, 0, 0, false, false, Look.color) ]);
  }
...
__decorate([ ViewRef({
  ...
  initView(layerListView) {
    ...
    const layer1 = layerListView.appendChild(RowView).backgroundColor(null);
    layer1.leaf.insertView().backgroundColor(null);
    layer1.getOrCreateCell("icon", IconCellView).iconWidth(24).iconHeight(24).graphics(this.owner.layersIcon);
    layer1.getOrCreateCell("title", TextCellView).content("Layer 1");
    layer1.on("click", (() => {
    ...
    layer2.getOrCreateCell("icon", IconCellView).iconWidth(24).iconHeight(24).graphics(this.owner.layersIcon);
    layer2.getOrCreateCell("title", TextCellView).content("Layer 2");
    layer2.on("click", (() => {
      const deckView = this.owner.deck.view;
      if (deckView !== null) {
        const layerCard = DeckCard.create();
    ...
    layer3.getOrCreateCell("icon", IconCellView).iconWidth(24).iconHeight(24).graphics(this.owner.layersIcon);
    layer3.getOrCreateCell("title", TextCellView).content("Layer 3");
    layer3.on("click", (() => {
      const deckView = this.owner.deck.view;
      if (deckView !== null) {
        const layerCard = DeckCard.create();
...
__decorate([ Lazy ], AtlasLayers, "layersListLayout", null);

__decorate([ Lazy ], AtlasLayers, "layersIcon", null);
...
__decorate([ Lazy ], AtlasPlugin, "icon", null);
...
__decorate([ EventDownlinkFastener({
  consumed: true,
  nodeUri() {
...
__decorate([ ListDownlinkFastener({
  consumed: true,
  nodeUri() {
    return this.owner.ownNodeUri;
  },
...
__decorate([ MapDownlinkFastener({
  ...
  didUpdate(key, newValue, oldValue) {
    this.owner.onEvent(Attr.of("update", key).concat(newValue));
  },
  didRemove(key, oldValue) {
    this.owner.onEvent(Record.of(Attr.of("remove", key)));
  }
...
__decorate([ ViewRef({
  ...
  type: IconCellView,
  createView() {
    const iconCellView = IconCellView.create().iconWidth(36).graphics(PolygonIcon.create(4, Math.PI / 4));
    return iconCellView;
  }
...
class InventoryView extends HtmlView {
  constructor(node) {
    super(node);
    this.masterLayout = new ConstraintGroup(this).constrain();
    this.mobileLayout = new ConstraintGroup(this);
...
class InventoryController extends ActivityController {
  ...
  createMasterLayout() {
    return TableLayout.create([ ColLayout.create("icon", 0, 0, 64), ColLayout.create("name", 1, 1, 200), ColLayout.create("kind", 0, 0, 64) ]);
  }
...
InventoryController.ItemRef = ControllerRef.define("ItemRef", {
  type: LaneController,
  observes: true,
...
__decorate([ Lazy ], InventoryController, "closeIcon", null);
...
export { ActivityController, ActivityPlugin, ActivityWindow, AgentsEntity, AtlasController, AtlasEntityDistrict, AtlasEntityLocation, AtlasLayers, AtlasMap, AtlasMapbox, AtlasPlugin, AtlasWorldMap, BeamView, CatalogController, CatalogEntityLimb, CatalogEntityTree, CatalogIndicatorCell, CatalogPlugin, ChartGadgetAreaPlotController, ChartGadgetBubblePlotController, ChartGadgetController, ChartGadgetLinePlotController, CollectionController, CollectorController, DiagramController, DiagramPlugin, DistrictTrait, DomainGroup, DomainPlugin, DomainTrait, DownlinkDistrictTrait, DownlinkLocationTrait, DownlinkNodeGroup, EntityGroup, EntityPlugin, EntityTrait, EventLaneModel, FabricEntity, FabricGroup, FabricPlugin, GadgetPlugin, GaugeGadgetController, GaugeGadgetDialController, Geographic, GeographicArea, GeographicAreaView, GeographicGroup, GeographicGroupView, GeographicLine, GeographicLineView, GeographicPoint, GeographicPointView, GeographicView, IndicatedGroup, IndicatedTrait, IndicatorGroup, IndicatorMap, IndicatorTrait, IndicatorType, InventoryController, InventoryPlugin, InventoryView, LaneController, LaneModel, ListLaneModel, LocationTrait, MagnifierController, MagnifierHandle, ManifestModel, MapLaneModel, MetaEdgeEntity, MetaHostEntity, MetaHostGroup, MetaMeshEntity, MetaMeshGroup, MetaNodeEntity, MetaNodeGroup, MetaPartEntity, MetaPartGroup, MirrorController, MirrorPlugin, NodeGroup, PieGadgetController, PieGadgetSliceController, PlaneEntity, PlaneGroup, PortalController, PortalPlugin, PrismPlugin, PrismProvider, PrismService, PulseTrait, ReflectionController, ReflectorController, RefractionController, RefractorController, SessionModel, ShellController, ShellView, Status, StatusFactor, StatusGroup, StatusTrait, StatusVector, SuggestionController, SurfaceView, TableGadgetController, TableGadgetIconCellController, TableGadgetRowController, TableGadgetTextCellController, ValueIndicatorTrait, ValueLaneModel, WidgetCard, WidgetController, WidgetGroup, WidgetTrait, WidgetView };
//# sourceMappingURL=swim-platform.mjs.map
